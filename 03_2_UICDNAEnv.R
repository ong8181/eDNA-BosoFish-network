...
#### No.3-2: Detecting causal pairs using UIC (Env xmap DNA)
####
...
for (env_i in env_var){
  ...
  x_std <- data.frame(x = as.numeric(scale(sample_df_sort[env_i])))
  
  # Parallel computing for eDNA taxa
  uic_env_edna_tmp <- pforeach(col_j = 1:length(top_taxa), .c=rbind)({
    ## Prepare output object
    uic_env_edna00 <- data.frame()
    fish_ts_i <- asv_df_conv_sort %>% select(top_taxa[col_j])
    ## Prepare causal variable (eDNA data)
    y_std <- data.frame(y = as.numeric(scale(fish_ts_i)))
    ...
    for (tp_i in TP_RANGE){
      uic_res_tmp <- rUIC::uic(cbind(x_std, y_std), lib_var = "x", tar_var = "y",
        ...
        cbind(data.frame(effect_var = env_i,
                         cause_var = top_taxa[col_j]), .)
      uic_env_edna00 <- rbind(uic_env_edna00, uic_res_tmp)
    }
  ...
  # Combine results
  uic_env_edna0 <- rbind(uic_env_edna0, uic_env_edna_tmp)
...
saveRDS(uic_env_edna0, sprintf("%s/uic_env_edna.obj", output_folder03))
